...
router.get("/all", (req, res, next) => {
    req.required_level = access_limitations.system_min_access_required.view_users_details;
...
router.post("/create", (req, res, next) => {
    req.required_level = access_limitations.system_min_access_required.create_new_user_with_specific_role;
    req.action_on_reject = _ => {
        res.redirect('/403');
    };
    next();
...
router.post("/modify/:username", (req, res, next) => {
    ...
    if (!current_password) {
        req.required_level = access_limitations.system_min_access_required.modify_all_users;
    } else {
        req.required_level = access_limitations.system_min_access_required.modify_current_user;
    }
    req.action_on_reject = _ => {
        res.redirect('/403');
    };
...
router.post("/remove/:username", (req, res, next) => {
    req.required_level = access_limitations.system_min_access_required.delete_different_users;
    req.action_on_reject = _ => {
        res.redirect('/403');
    };
    next();
